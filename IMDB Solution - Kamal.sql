...
-- Type your code below:
    ...
    SELECT
        genre,
        COUNT(movie_id) AS movies_produced
    FROM
        genre
    GROUP BY
        genre
...
-- Type your code below:
    ...
    SELECT
        COUNT(*) AS movies_belonging_to_one_genre
    FROM
        (
            SELECT
                movie_id
...
/*
 Since, the average rating was the only deciding criteria for the Top 10 movies and no further clarification was provided,
 2 solutions have been provided as mentioned below.

-- Solution 1:
Wherein multiple movies belonging to the same rank are considered since the avg_rating was the only criteria and 
it wasn't correct to dismiss the other movies who passed the criteria.
 */
...
/*
-- Solution 2:
Wherein multiple movies belonging to 1 rank are ignored and the number of movies/results is limited to 10.
*/
    SELECT
       title,
       avg_rating,
...
-- Type your code below:
    ...
    FROM
        ...
        JOIN ratings r ON m.id = r.movie_id
    WHERE
        DATE_FORMAT(date_published, '%M %Y') = 'March 2017' AND
...
-- Type your code below:
    ...
    SELECT
        COUNT(id) AS movie_count_with_median_rating_of_8
    FROM
        movie m
        JOIN ratings r ON m.id = r.movie_id
    WHERE
...
-- Type your code below:
    ...
    FROM genre
        JOIN ratings USING(movie_id)
        JOIN director_mapping USING(movie_id)
...
-- Type your code below:
    ...
    SELECT
        ...
        DENSE_RANK() over (ORDER BY SUM(total_votes * avg_rating) / SUM(total_votes) DESC) AS actor_rank
    FROM
        movie m
...
-- Type your code below:
    ...
    WITH actress_ratings AS (
        ...
        FROM
            ...
            JOIN ratings r ON movie.id = r.movie_id
        WHERE
            country LIKE '%India%' AND
        ...
        movie_earnings AS (
            SELECT
                ...
                DENSE_RANK() over (
                    PARTITION BY year
                    ORDER BY
                        -- Converting INR to USD when the amount starts with INR
                        IF(worlwide_gross_income LIKE 'INR%',
                           CASE
                               WHEN year = 2017 THEN
                                   -- Removing any non-numeric characters
                                   CAST(REGEXP_REPLACE(worlwide_gross_income, '[^0-9.]', '') AS DECIMAL(10)) * 0.01536 -- 2017: 1 INR = 0.01536 USD
                               WHEN year = 2018 THEN
                                   -- Removing any non-numeric characters
                                   CAST(REGEXP_REPLACE(worlwide_gross_income, '[^0-9.]', '') AS DECIMAL(10)) * 0.01461 -- 2018: 1 INR = 0.01461 USD
                               WHEN year = 2019 THEN
                                   -- Removing any non-numeric characters
                                   CAST(REGEXP_REPLACE(worlwide_gross_income, '[^0-9.]', '') AS DECIMAL(10)) * 0.01420 -- 2019: 1 INR = 0.01420 USD
                            END,
                           -- Removing any non-numeric characters for non-INR amounts
                           CAST(REGEXP_REPLACE(worlwide_gross_income, '[^0-9.]', '') AS DECIMAL(10))) DESC
                ) AS movie_rank
            FROM
                movie m
                JOIN genre g ON m.id = g.movie_id
...
-- Type your code below:
    ...
    WITH top_actresses_in_drama AS (
        SELECT
            name AS actress_name,
            SUM(total_votes) AS total_votes,
...
-- Type you code below:
    ...
    WITH publisher_details AS
        (
            SELECT d.name_id, name, d.movie_id, duration, r.avg_rating, total_votes, m.date_published,
                    LEAD(date_published,1) OVER(partition BY d.name_id ORDER BY date_published, d.movie_id) AS next_date
